...
if [ $# -ne 1 ]; then
    ...
    source "$SRC_DIR/BananaOS/configs/version.sh" || exit 1
...
if [ ! "$TARGET_OS_SINGLE_SYSTEM_IMAGE" ]; then
    ...
    exit 1
elif [ ! -f "$SRC_DIR/BananaOS/configs/$TARGET_OS_SINGLE_SYSTEM_IMAGE.sh" ]; then
    LOGE "\"$TARGET_OS_SINGLE_SYSTEM_IMAGE\" is not a valid system image"
    exit 1
else
    source "$SRC_DIR/BananaOS/configs/$TARGET_OS_SINGLE_SYSTEM_IMAGE.sh" || exit 1
fi
